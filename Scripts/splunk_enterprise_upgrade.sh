...
detect_service_manager() {
    ...
    if ! command -v systemctl &>/dev/null; then
        ...
        track_step "service_manager" "PASS" "cli (no systemctl)"; return
    fi

    # `systemctl is-system-running` is NOT the right gate here: it reports
    # "degraded" (nonzero exit) whenever ANY unrelated unit on the host has
    # failed, which is common in the real world and has nothing to do with
    # whether systemd can manage the Splunk unit. The reliable check is
    # whether systemd is actually the running init system.
    if [ ! -d /run/systemd/system ]; then
        log_info "Service manager: Splunk CLI  (systemd not running)"
        _log_raw "[SVC]  method=cli  reason=systemd_not_running"
        track_step "service_manager" "PASS" "cli (systemd not running)"; return
    fi

    local sys_state
    sys_state=$(systemctl is-system-running 2>/dev/null || true)
    if [ "${sys_state}" != "running" ]; then
        log_warn "systemd state: ${sys_state:-unknown}  (proceeding — unrelated units may be degraded)"
        _log_raw "[SVC]  systemd_state=${sys_state:-unknown}  (non-fatal, continuing)"
    fi
    ...
    if found=$(_find_unit); then
        ...
        track_step "service_manager" "PASS" "systemd (existing unit)"; return
    fi
...
_splunk_start_with_license() {
    # systemd path: persist license acceptance without starting the process,
    # then issue a single authoritative systemctl start. This prevents a
    # double-start (P0-03 from review).
    #
    # IMPORTANT: `splunk --accept-license --answer-yes --no-prompt` with NO
    # subcommand is not a valid invocation — Splunk's CLI rejects it outright
    # ("'-no-prompt' is not a valid command") and accepts nothing. `version`
    # is a real subcommand that accepts the license and exits immediately
    # without starting the daemon. It ALSO still needs --answer-yes: any CLI
    # invocation right after an RPM upgrade triggers Splunk's one-time config
    # migration prompt ("Perform migration and upgrade without previewing
    # configuration changes? [y/n]"), and --no-prompt without --answer-yes
    # aborts on that question ("Exiting because user requested no prompting")
    # instead of auto-confirming it — this must never be dropped.
    #
    ...
    if [ "${USE_SYSTEMD}" = true ]; then
        local lic_out lic_code
        if lic_out=$(su - "${SPLUNK_USER}" -c "${SPLUNK_BIN} version --accept-license --answer-yes --no-prompt" 2>&1); then
            lic_code=0
        else
            lic_code=$?
        fi
        printf '%s\n' "${lic_out}" >> "${LOG_FILE}"
        if [ "${lic_code}" -ne 0 ]; then
            printf '%s\n' "${lic_out}"
            return "${lic_code}"
        fi
...
select_package() {
    ...
    if [ "${has_latest}" -eq 0 ]; then
        box_option "1" "Splunk Enterprise — Latest version (from detected URL)"
        box_line "  ${LATEST_LABEL}" 4
        box_empty
        box_option "2" "Enter an approved download URL"
        box_option "3" "Provide a path to a local RPM file"
    else
    ...
    while true; do
        read -rp "${LYELLOW}  [?] Choice: ${Color_Off}" choice
        [[ "${choice}" =~ ${valid_range} ]] && break
        log_warn "Enter a valid option number."
    done

    # Normalize to the fixed 3-way branch below regardless of which numbers
    # were shown (the "latest" option only appears when one was detected).
    local action="url_prompt"
    if [ "${has_latest}" -eq 0 ]; then
        case "${choice}" in
            1) action="latest" ;;
            2) action="url_prompt" ;;
            3) action="local_prompt" ;;
        esac
    else
        case "${choice}" in
            1) action="url_prompt" ;;
            2) action="local_prompt" ;;
        esac
    fi

    if [ "${action}" = "latest" ]; then
        SPLUNK_DOWNLOAD_URL="${LATEST_URL}"
        PACKAGE_NAME=$(basename "${LATEST_URL}")
        PACKAGE_PATH="${WORK_DIR}/${PACKAGE_NAME}"
        SELECTED_VERSION="latest (detected): ${LATEST_URL}"
    elif [ "${action}" = "url_prompt" ]; then
        echo ""
        box_top
...
step_start_splunk() {
    ...
    # The first start after a version upgrade can run heavy one-time
    # migration work (KV store version bump, datamodel/report acceleration
    # checksum regeneration across every index, DFS/peer-apps migration).
    # Under systemd's default Restart=always rate limiting, a slow first
    # attempt can trip "Start request repeated too quickly" and the unit
    # gives up in a *failed* state before migration finishes — even though
    # the exact same work succeeds given a clean run. Retry a bounded
    # number of times, clearing the failed state between attempts, so a
    # rate-limit trip doesn't get reported as a hard upgrade failure.
    ...
    while true; do
        ...
        out="${_CAP_OUT}"
        code="${_CAP_CODE}"
        _log_raw "[START]  attempt=${attempt}/${max_attempts}  exit=${code}  ${out}"

        [ "${code}" -eq 0 ] && break
        [ "${attempt}" -ge "${max_attempts}" ] && break

        log_warn "Start attempt ${attempt}/${max_attempts} failed [exit: ${code}] — retrying ..."
        if [ "${USE_SYSTEMD}" = true ]; then
            systemctl reset-failed "${SYSTEMD_SERVICE}" 2>/dev/null || true
        fi
        sleep 10
        attempt=$(( attempt + 1 ))
    done
    ...
    if [ "${code}" -ne 0 ]; then
        log_error "Splunk start failed after ${attempt} attempt(s)  [exit: ${code}]"
        log_error "See: ${LOG_FILE}"
        log_to_file "start" "_splunk_start_with_license" "${out}"
        if [ "${USE_SYSTEMD}" = true ]; then
            log_error "Recent systemd journal for ${SYSTEMD_SERVICE}:"
            local jrnl
            jrnl=$(journalctl -u "${SYSTEMD_SERVICE}" -n 50 --no-pager 2>&1 || true)
            printf '%s\n' "${jrnl}" | tee -a "${LOG_FILE}"
        fi
        if [ -f "${SPLUNK_HOME}/var/log/splunk/splunkd.log" ]; then
            log_error "Last 30 lines of splunkd.log:"
            tail -30 "${SPLUNK_HOME}/var/log/splunk/splunkd.log" | tee -a "${LOG_FILE}"
        fi
        track_step "start" "FAILED"; exit 1
    fi

    [ "${attempt}" -gt 1 ] && log_warn "Start succeeded on attempt ${attempt}/${max_attempts}."
...
print_summary() {
    ...
    local ordered_keys=(
        ...
        service_manager topology package_select connectivity download
    ...
    local labels=(
        ...
        "User / Group"          "Installed RPM"        "Service Manager"
        "Topology"              "Package Selection"    "Connectivity"
        "Download"              "RPM Metadata"         "Disk Space"
